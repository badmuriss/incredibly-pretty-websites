...
> **Source:** [github.com/nexu-io/open-design](https://github.com/nexu-io/open-design) `craft/laws-of-ux.md` @ `f580271`, Apache-2.0. **Edited for this repo:** cross-references to open-design sibling files were rewritten to point at the matching SKILL.md sections, and the loading-threshold table was inlined because no sibling file carries it here. §13 of SKILL.md wins over anything below.
...
earns its place, why a settings list needs grouping. The brand
research and the DESIGN.md pack decide the brand visual language;
SKILL.md decides rendering rules (palette, type, motion, states, ARIA)
and [form-validation.md](form-validation.md) decides form behaviour;
this file decides composition rules grounded in named research.
...
for an HTML / Tailwind / React-emitting agent. The enforced rules live
in SKILL.md (§4 Rule 5 interaction states, §6 Animation engine, §11
Accessibility, §12 Type catalog, §13 AI TELLS, §15 Color / contrast)
and in [form-validation.md](form-validation.md); this file names the
...
apply them; the linter does not. Where a law is already enforced
elsewhere in this skill — the touch-target floor in §11, the 8 required
interaction states in §4 Rule 5, the approved families in §12, the
forbidden-pattern list in §13 — the entry below cross-references rather
than duplicates. Where nothing here carries the rule, the entry states
it inline. **§13 (AI TELLS) outranks every entry below.**
...
- **Von Restorff Effect** (von Restorff, 1933). The item that differs
  ...
  label, position). Color alone is never the signal: WCAG 1.4.1 rules it
  out and §11 assumes it.
- **Aesthetic-Usability Effect** (Kurosu & Kashimura, Hitachi Design
  Center, 1995). Visual polish biases perceived usability. Refined
  type, generous whitespace, and a calm palette earn the benefit
  of the doubt for minor friction. Never substitutes for measurable
  usability or for §4 Rule 5's eight required interaction states.
...
- **Peak-End Rule** (Kahneman, Fredrickson, Schreiber, Redelmeier,
  *Psychological Science*, 1993). Memory of an experience is dominated
  ...
  belongs at the *end* of a flow, not as arbitrary mid-flow motion. §6
  ("Should this animate?") rejects motion that performs rather than
...
- **Fitts's Law** (Fitts, *Journal of Experimental Psychology*, 1954).
  ...
  much as size. §11 sets 44 × 44 px on `(pointer: coarse)`, above the
  WCAG 2.2 AA floor of 24 × 24 CSS px; on mobile, place high-frequency
...
- **Doherty Threshold** (Doherty & Thadani, *IBM Systems Journal*,
  ...
  breaks attention. Nothing else in this skill carries the threshold
  table, so it lives here: no indicator under 300 ms; skeleton
  300 ms – 2 s; labelled spinner 2 – 10 s; determinate bar with cancel
  10 – 60 s; stop and offer error/retry past 60 s. §4 Rule 5 already
  prefers a skeleton matching the layout over a generic spinner, which is
  the 300 ms – 2 s row. Folklore note: the 400 ms number often quoted as
  "the Doherty threshold" does not appear in the 1982 paper.
...
- **Cognitive Load** (Sweller, *Cognitive Science*, 1988). Total
  ...
  fully. The visual-restraint directives already in this skill (accent
  and contrast discipline in §15, the curated type catalog and pairing
  rules in §12, the forbidden-pattern list in §13) constrain extraneous
  load; this entry names the cognitive cost those rules reduce.

## Common mistakes (lint these)

Folklore corrections that don't survive a primary-source check. The
first three are attribution
...
- "Fitts's Law alone is enough for touch targets." Fitts gives the
  ...
  48 × 48 dp, and §11 of this skill requires 44 × 44 px on coarse
  pointers. Fitts plus the platform floor — never just Fitts.
