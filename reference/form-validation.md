...
> **Source:** [github.com/nexu-io/open-design](https://github.com/nexu-io/open-design) `craft/form-validation.md` @ `f580271`, Apache-2.0. **Edited for this repo:** cross-references to open-design sibling files were rewritten to point at the matching SKILL.md sections, and the accessible baseline error pattern was inlined because no sibling file carries it here. §13 of SKILL.md wins over anything below.
...
DESIGN.md pack decides how the field looks; this file decides *when* the
...
truth.** The a11y baseline for a field is the pattern SKILL.md §11 and
[interaction-design.md](interaction-design.md) already require: a real
visible `<label>` (never a placeholder), the error rendered below the
field, wired with `aria-describedby` + `aria-invalid="true"`, and
`role="alert"` on the inline error node. This file picks up where that
ends.
...
That baseline pattern (`<label>` + `aria-describedby` + `aria-invalid`
+ `role="alert"`) covers WCAG 3.3.1 / 3.3.2. Three additions matter for
real forms:
...
the canonical baseline pattern above. WCAG
...
The baseline pattern above covers 3.3.1 (Error ID) and 3.3.2 (Labels),
and dropping email-confirm fields covers 3.3.7 (Redundant Entry). The
rest of 3.3 binds harder on transactional forms:
